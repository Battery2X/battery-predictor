...
# v16.5 개선사항
# ============================================================
# [개선 1] P/R 이중 필터 추가 (HIGH 우선순위)
#   - 기존: F1 > 0.50만 체크
#   - 문제: P 34%, R 42% 신호가 F1 통과 후 출력됨 (XOVR 등)
#   - 수정: F1 > 0.50 AND P > 0.48 AND R > 0.48 삼중 조건
#   - 효과: 낮은 P/R 신호 자동 필터링
#
# [개선 2] 신호 연속성 추적 — 전일 비교 (HIGH 우선순위)
#   - 기존: 매일 독립 신호 출력
#   - 수정: 전일 신호 JSON 저장 → 오늘과 비교 → 반전 시 ⚠️ FLIP 출력
#   - 효과: IONQ 이격도 급변 등 중요 변화 즉시 인지
#
# [개선 3] 이벤트 캘린더 자동 경고 (MID 우선순위)
#   - FOMC, CPI, 실적 날짜 하드코딩
#   - 이벤트 D-1, D-0 자동 경고 출력
#
# [개선 4] 나스닥 MA200 라벨 수정 (LOW 우선순위)
#   - "MA200 이격" → "나스닥 MA200 이격" 명확화
# ============================================================
...
def check_upcoming_events():
    ...
    for date_str, event in MARKET_EVENTS.items():
        ...
        if 0 <= days_left <= 2:
            ...
            warnings_list.append(f"  {prefix}: {event}")
...
def detect_signal_flip(ticker, today_sig, prev_history):
    ...
    if ticker not in prev_history:
        return None
    prev = prev_history[ticker]
    prev_sig = prev.get('signal', '')
    prev_disp = prev.get('disparity', None)

    flip_msg = []

    # 방향 반전 감지
    up_signals = ['강한상승', '약한상승', '우상향']
    dn_signals = ['강한하락', '약한하락', '하락우세']

    prev_dir = 'up' if any(s in prev_sig for s in up_signals) else ('dn' if any(s in prev_sig for s in dn_signals) else 'neutral')
    now_dir  = 'up' if any(s in today_sig for s in up_signals) else ('dn' if any(s in today_sig for s in dn_signals) else 'neutral')

    if prev_dir != now_dir and prev_dir != 'neutral' and now_dir != 'neutral':
        arrow = '🔄'
        flip_msg.append(f"{arrow} 신호 반전: {prev_sig} → {today_sig}")

    # 이격도 급변 감지 (양자주)
    if prev_disp is not None and 'disparity' in prev:
        curr_disp = prev_history.get(ticker, {}).get('disparity_today')
        if curr_disp is not None:
            disp_chg = abs(curr_disp - prev_disp)
            if disp_chg >= 8:
                flip_msg.append(f"📊 이격도 급변: {prev_disp:.1f}% → {curr_disp:.1f}% ({disp_chg:+.1f}%p)")

    return '\n'.join(flip_msg) if flip_msg else None
...
try:
    print("🚀 v16.5 US AI·반도체 단기 통제소 가동...")
    print("   [개선: P/R 필터 | 신호반전 감지 | 이벤트 캘린더 | 라벨 수정]\n")
    ...
    macro_tickers = {
        'Nasdaq': '^IXIC', 'SMH': 'SMH', 'VIX': '^VIX', 'TNX': '^TNX'
    }
    macro_data = {}
    ...
    final_report = "🤖 [US AI·반도체 올인 방향성 통제소 v16.5]\n"
    final_report += "=" * 40 + "\n"
    ...
    for ticker in targets:
        ...
        # [개선 1] P/R 이중 필터 — 핵심 개선
        MIN_F1   = 0.50
        MIN_PR   = 0.48  # P와 R 모두 이 값 이상이어야 신호 유효
        ...
        if not f1_pass or not pr_pass:
            fail_reason = []
            if not f1_pass: fail_reason.append(f"F1 {f1*100:.1f}%")
            if not pr_pass: fail_reason.append(f"P {prec*100:.0f}%/R {rec*100:.0f}% 미달")

            today_signal = "관망"
            today_history[ticker] = {
                'signal': '관망',
            ...
            final_report += f"  * ⚠️ 관망 ({' | '.join(fail_reason)})\n"
            if flip_msg:
        ...
        # [개선 4] 나스닥 MA200 라벨 명확화 (장기 모델 아니므로 참고용 출력)
        final_report += f"📌 {ticker}\n"
